...
def make_train(config, env):
    ...
    def train(rng):
        ...
        def create_agent(rng):
            ...
            network_params = network.init(rng, init_batch_hs, *init_batch_x, init_batch_hs)
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env(carry, _):
                ...
                timestep = Timestep(
                    ...
                    dones=last_dones, #fix to last dones
                    avail_actions=avail_actions,
                )
            ...
            expl_state = (init_hs, init_obs, init_dones, init_hs, env_state)
            rng, _rng = jax.random.split(rng)
            ...
            def _learn_phase(carry, _):
                ...
                _, q_next_target, pre_hs = network.apply(train_state.target_network_params, init_hs,
                                                 _obs, _dones, init_hs)


                def _loss_fn(params):
                    # print(f"In loss init_hstate shape: {init_hs.shape}")
                    # print(f"In loss obs shape: {_obs.shape}")
                    # print(f"In loss done shape : {_dones.shape}")
                    _, q_vals, pre_hs = network.apply(params, init_hs, _obs, _dones, init_hs)
...
def single_run(config):
    ...
    if config['ENV_KWARGS']['train_pre_policy']:
        alg_name = "Pre_policy_" + alg_name
    else:
        alg_name = "Baseline_" + alg_name
    wandb.init(
...
def tune(default_config):
    ...
    alg_name = default_config.get("ALG_NAME", "vdn_our_method")
    ...
    def wrapped_make_train():
        ...
        train_vjit = jax.jit(jax.vmap(make_train(config, env)))
        outs = jax.block_until_ready(train_vjit(rngs))
    config_name = default_config.get("CONFIG_NAME", "")


    sweep_config = {
        "name": f"{alg_name}_{config_name}_{env_name}",
        "method": "grid",
        "metric": {
